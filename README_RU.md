...
## To-Do
- [ ] Реализовать чтение данных из стандартного ввода для использования в цепочках команд Unix (например, `cat list.txt | prefixopt optimize`).
- [ ] `expand` Развертывание CIDR префиксов в список отдельных IP-адресов.
- [ ] Возможность разбивать вывод на несколько файлов по количеству строк или хостов (для задач сканирования).
- [ ] Интеграция библиотеки `ijson` для обработки больших JSON-файлов без полной загрузки в память (снижение потребления RAM).
